...
#' **mfuzz**, **TCseq**, **kmeans**, or **wgcna**. The clustering results
#' include
...
#'   - **wide.res**: A wide-format data frame with clusters and normalized
#'   expression levels.
#'   - **long.res**: A long-format data frame for visualizations, containing
#'   cluster information,
#'                   normalized values, cluster names, and memberships.
#'   - **cluster.list**: A list where each element contains genes belonging to
#'    a specific cluster.
#'   - **type**: The clustering method used (`"mfuzz"`, `"TCseq"`, `"kmeans"`,
#'    or `"wgcna"`).
...
#'   - **`"TCseq"`**: Uses TCseq clustering for time-series expression data
#'   with support for additional parameters.
#'   - **`"kmeans"`**: Employs standard k-means clustering via base R's
#'   `stats::kmeans`.
#'   - **`"wgcna"`**: Leverages pre-calculated WGCNA (Weighted Gene
...
#' If the **WGCNA** method is selected, the `object` parameter must contain
#' a pre-calculated WGCNA network object.
#' This is typically obtained using the WGCNA package functions.
#'
#' @section Subsetting Clusters:
#' Use the `subcluster` parameter to focus on specific clusters. Cluster IDs
#'  not included in the
...
clusterData <- function(obj = NULL,
  ...
  if (cluster.method %in% c("mfuzz", "TCseq")) {
    ...
    df$cluster_name <- factor(df$cluster_name,
      levels =
        paste("cluster ", seq_len(nrow(cl.info)), " (",
              cl.info$Freq, ")", sep = "")
    )
    ...
    # cluster order
    df$cluster_name <- factor(df$cluster_name,
      levels = paste("cluster ", seq_len(nrow(cl.info)),
                     " (", cl.info$Freq, ")", sep = "")
    )
    # cluster list
    wide <- wide.r
    ...
    # cluster order
    # df$cluster_name <- factor(df$cluster_name,
    # levels = paste('cluster ',1:nrow(cl.info),sep = ''))
    ...
    # cluster order
    # df$cluster_name <- factor(df$cluster_name,
    # levels = paste("cluster ",1:nrow(cl.info),
    #                " (",cl.info$Freq,")",sep = ''))

    df$cluster_name <- factor(
      df$cluster_name,
      levels = paste(
        "cluster ",
        seq_len(nrow(cl.info)),
        " (",
        cl.info$Freq,
        " ",
        unique(df$modulecol),
